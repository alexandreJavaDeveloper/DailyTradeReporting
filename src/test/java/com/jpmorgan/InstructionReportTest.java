...
public class InstructionReportTest
{
    ...
    @Test
    public void testIncorrectShowReport()
    {
        final InstructionReport report = new InstructionReport();

        final List<Instruction> localTrades = new ArrayList<>();

        // empty Trades should work
        report.showReport(localTrades);

        LocalDate settlementDate = LocalDate.of(2017, 2, 26);
        Instruction instruction = new Instruction("foo", FinancialType.BUY, 0.50f, CurrencyType.SAR, LocalDate.now(), settlementDate, 200, 100.25f);
        localTrades.add(instruction);

        // only BUY FinancialType should work
        report.showReport(localTrades);

        localTrades.clear();

        settlementDate = LocalDate.of(2017, 2, 26);
        instruction = new Instruction("foo", FinancialType.SELL, 0.50f, CurrencyType.SAR, LocalDate.now(), settlementDate, 200, 100.25f);
        localTrades.add(instruction);

        // only SELL FinancialType should work
        report.showReport(localTrades);
    }
}